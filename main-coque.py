...
# =====================================================
# Coqui TTS — Voice catalogue
# =====================================================
#
# Models used:
#   English : tts_models/en/ljspeech/tacotron2-DDC  (~150 MB)
#             Single high-quality female voice.
#
#   Hindi   : tts_models/hi/cv/vits                 (~80 MB)
#             Single Hindi VITS voice.
#
# Both models download automatically on first use,
# then run 100% offline. Requires Python 3.12.
#
# NOTE: Coqui TTS has single-speaker models only —
#       each model = one voice. Male/Female is per model.
# =====================================================
...
TTS_MODELS = {
    "en_female": {
        ...
        "gender":  "Female",
        "name":    "LJSpeech",
        "label":   "LJSpeech (English Female, Clear & Natural)",
    },
    "en_male": {
        ...
        "gender":  "Male",
        "name":    "VCTK p273",
        "label":   "VCTK p273 (English Male, Neutral)",
        "speaker": "p273",          # VCTK is multi-speaker; pick a male speaker
    },
    "hi_neutral": {
        "model":   "tts_models/hi/cv/vits",
        "lang":    "hi",
        "gender":  "Female",
        "name":    "Hindi VITS",
        "label":   "Hindi VITS (Hindi, Natural)",
    },
}

# Frontend voice list
TTS_VOICES = [
    {"voice_id": k, "lang": v["lang"], "gender": v["gender"],
     "name": v["name"], "label": v["label"]}
    for k, v in TTS_MODELS.items()
]

# Cache loaded TTS instances  { voice_id -> TTS instance }
_tts_cache: Dict[str, object] = {}


def get_tts(voice_id: str):
    """Return (and memory-cache) a Coqui TTS instance for *voice_id*."""
    if voice_id not in TTS_MODELS:
        raise ValueError(f"Unknown voice_id '{voice_id}'.")
    if voice_id not in _tts_cache:
        from TTS.api import TTS as CoquiTTS
        model_name = TTS_MODELS[voice_id]["model"]
        logger.info(f"Loading Coqui model '{model_name}' — first run downloads the model…")
        _tts_cache[voice_id] = CoquiTTS(model_name=model_name, progress_bar=True)
        logger.info(f"Coqui model '{voice_id}' ready.")
    return _tts_cache[voice_id]


def synthesize_coqui(text: str, voice_id: str, speed: float, pitch: float) -> str:
    """
    1. Synthesize text → WAV  (Coqui TTS, offline)
    2. Post-process speed + pitch via ffmpeg
    3. Encode to 192 kbps MP3
    Returns absolute path to MP3 in AUDIO_DIR.
    """
    meta    = TTS_MODELS[voice_id]
    tts     = get_tts(voice_id)
    uid     = uuid.uuid4().hex
    wav_out = os.path.join(TEMP_DIR,  f"tts_{uid}.wav")
    mp3_out = os.path.join(AUDIO_DIR, f"tts_{uid}.mp3")

    logger.info(f"Coqui TTS: voice={voice_id} | speed={speed} | pitch={pitch} | chars={len(text)}")

    # Use speaker arg only for multi-speaker models (e.g. VCTK)
    speaker = meta.get("speaker")
    if speaker:
        tts.tts_to_file(text=text, file_path=wav_out, speaker=speaker)
    else:
        tts.tts_to_file(text=text, file_path=wav_out)

    logger.info(f"WAV saved: {wav_out}")

    # Post-process: speed via atempo, pitch via asetrate
    filters = []
    speed = max(0.5, min(2.0, float(speed)))
    if speed != 1.0:
        filters.append(f"atempo={speed:.3f}")
    if pitch != 0.0:
        base_rate      = 22050                        # Coqui default sample rate
        semitone_ratio = 2 ** (float(pitch) / 12.0)
        filters.append(f"asetrate={int(base_rate * semitone_ratio)}")
...
def transcribe_with_assemblyai(audio_path: str) -> str:
    try:
        ...
        config = aai.TranscriptionConfig(
            speech_model=aai.SpeechModel.best,   # was .universal in older SDK
            language_code="hi"
        )
...
async def health_check():
    return {
        "status": "ok",
        "tts_models_loaded": list(_tts_cache.keys())
    }
...
# =====================================================
# TTS routes — Coqui TTS (free, offline, neural)
# =====================================================

class TTSRequest(BaseModel):
    text:     str
    voice_id: str   = "en_female"   # en_female | en_male | hi_neutral
...
async def list_voices():
    """Return all available Coqui TTS voices."""
...
async def tts_synthesize(req: TTSRequest):
    """
    Generate an MP3 using Coqui TTS (free, fully offline).
    First call per voice downloads the model (~80-150 MB).
    ...
    if req.voice_id not in TTS_MODELS:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown voice_id '{req.voice_id}'. Use: {list(TTS_MODELS.keys())}"
        )
