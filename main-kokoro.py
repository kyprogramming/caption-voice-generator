...
# =====================================================
# Kokoro TTS — Voice catalogue
# lang_code: 'a' = US English, 'b' = British English, 'h' = Hindi
# Voices: af_* = American Female, am_* = American Male
...
TTS_VOICES = [
    # English Female
    {"voice_id": "af_heart",   "lang_code": "a", "lang": "en", "gender": "Female", "name": "Heart",   "label": "Heart (US Female, Warm)"},
    {"voice_id": "af_sarah",   "lang_code": "a", "lang": "en", "gender": "Female", "name": "Sarah",   "label": "Sarah (US Female, Natural)"},
    {"voice_id": "af_nicole",  "lang_code": "a", "lang": "en", "gender": "Female", "name": "Nicole",  "label": "Nicole (US Female, Soft)"},
    {"voice_id": "af_sky",     "lang_code": "a", "lang": "en", "gender": "Female", "name": "Sky",     "label": "Sky (US Female, Airy)"},
    {"voice_id": "bf_emma",    "lang_code": "b", "lang": "en", "gender": "Female", "name": "Emma",    "label": "Emma (British Female)"},
    {"voice_id": "bf_isabella","lang_code": "b", "lang": "en", "gender": "Female", "name": "Isabella","label": "Isabella (British Female)"},
    # English Male
    {"voice_id": "am_adam",    "lang_code": "a", "lang": "en", "gender": "Male",   "name": "Adam",    "label": "Adam (US Male, Deep)"},
    {"voice_id": "am_michael", "lang_code": "a", "lang": "en", "gender": "Male",   "name": "Michael", "label": "Michael (US Male, Neutral)"},
    {"voice_id": "bm_george",  "lang_code": "b", "lang": "en", "gender": "Male",   "name": "George",  "label": "George (British Male)"},
    {"voice_id": "bm_lewis",   "lang_code": "b", "lang": "en", "gender": "Male",   "name": "Lewis",   "label": "Lewis (British Male)"},
    # Hindi Female
    {"voice_id": "hf_alpha",   "lang_code": "h", "lang": "hi", "gender": "Female", "name": "Alpha",   "label": "Alpha (Hindi Female)"},
    {"voice_id": "hf_beta",    "lang_code": "h", "lang": "hi", "gender": "Female", "name": "Beta",    "label": "Beta (Hindi Female)"},
    # Hindi Male
    {"voice_id": "hm_omega",   "lang_code": "h", "lang": "hi", "gender": "Male",   "name": "Omega",   "label": "Omega (Hindi Male)"},
    {"voice_id": "hm_psi",     "lang_code": "h", "lang": "hi", "gender": "Male",   "name": "Psi",     "label": "Psi (Hindi Male)"},
]

# Cache pipelines per lang_code so they load only once
...
def get_pipeline(lang_code: str):
    if lang_code not in _kokoro_pipelines:
        ...
        logger.info(f"Kokoro pipeline '{lang_code}' ready.")
    return _kokoro_pipelines[lang_code]


def synthesize_kokoro(text: str, voice_id: str, speed: float) -> str:
    """
    Synthesize with Kokoro TTS → WAV → convert to 192kbps MP3 via ffmpeg.
    Returns absolute path to MP3 in AUDIO_DIR.
    """
    # Look up voice metadata
    voice_meta = next((v for v in TTS_VOICES if v["voice_id"] == voice_id), None)
    if not voice_meta:
    ...
    uid     = uuid.uuid4().hex
    wav_out = os.path.join(TEMP_DIR,  f"tts_{uid}.wav")
    mp3_out = os.path.join(AUDIO_DIR, f"tts_{uid}.mp3")

    logger.info(f"Kokoro: voice={voice_id} | speed={speed} | chars={len(text)}")

    # Generate audio — Kokoro returns chunks; concatenate all
    all_audio = []
    sample_rate = 24000
    ...
    sf.write(wav_out, audio_np, sample_rate)
    logger.info(f"WAV saved: {wav_out}")
    ...
    try:
        (
            ffmpeg
            .input(wav_out)
            .output(mp3_out, audio_bitrate="192k", acodec="libmp3lame")
            .overwrite_output()
...
"""
        ...
        for i, block in enumerate(blocks):
            ...
            words = text.split()
            duration_per_word = 3.0 / max(len(words), 1)
            karaoke_line = ''.join([f"{{\\k{int(duration_per_word * 100)}}}{w} " for w in words])
            ass_lines.append(
                f"Dialogue: 0,{ass_start},{ass_end},Default,,0,0,0,,{{\\an5}}{karaoke_line.strip()}"
            )
...
def burn_subtitles(video_path, srt_file, output_path):
    ...
    try:
        ffmpeg.input(video_path).output(
            output_path,
            vf=f"ass={safe_ass}",
            crf=18, preset='ultrafast', tune='fastdecode',
            movflags='+faststart', vsync='vfr', threads=num_cores
...
def transcribe_with_assemblyai(audio_path: str) -> str:
    try:
        ...
        config = aai.TranscriptionConfig(speech_model=aai.SpeechModel.best, language_code="hi")
...
async def upload_video(file: UploadFile = File(...)):
    ...
    try:
        ...
        ffmpeg.input(video_path).output(audio_path, ac=1, ar=16000).overwrite_output().run(capture_stdout=True, capture_stderr=True)
        logger.info(f"Audio extracted: {audio_path}")
        loop     = asyncio.get_running_loop()
        srt_text = await loop.run_in_executor(executor, transcribe_with_assemblyai, audio_path)
        save_srt_from_text(srt_text, srt_path)
        uploads[uid] = {
            "video_path": video_path, "original_filename": fname,
            "srt_path": srt_path, "output_name": output_video_name, "output_path": output_path
        }
        return {"uid": uid, "original_filename": fname, "srt_url": f"/srt/{uid}", "video_url": f"/original/{uid}"}
    except Exception as e:
...
async def serve_original(uid: str):
    ...
    if not info:
        raise HTTPException(status_code=404, detail="UID not found")
    return FileResponse(info["video_path"], media_type="video/mp4", filename=info["original_filename"])
...
async def health_check():
    return {"status": "ok", "kokoro_pipelines_loaded": list(_kokoro_pipelines.keys())}
...
def log_current_dir():
    ...
    try:
        for item in os.listdir(cwd):
            ...
            logger.info(f"[{'DIR' if os.path.isdir(path) else 'FILE'}]  {path}")
    except Exception as e:
        logger.error(f"Error listing directory: {e}")


# =====================================================
# TTS routes — Kokoro (offline, neural quality)
# =====================================================
...
async def tts_synthesize(req: TTSRequest):
    ...
    valid = {v["voice_id"] for v in TTS_VOICES}
    ...
    try:
        ...
        mp3_path = await loop.run_in_executor(
            executor, synthesize_kokoro,
            req.text, req.voice_id, req.speed,
        )
